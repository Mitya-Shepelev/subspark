...
if (isset($_POST) && count($_POST) > 0) {
	...
	if ($validation === true) {
		$time = time();
    DB::exec(
        "INSERT INTO i_user_payments(payer_iuid_fk,order_key,payment_type,payment_option,payment_time, payment_status,credit_plan_id) VALUES (?,?,?,?,?,'pending',?)",
        [
            (int)$userID,
		...
		} elseif ($paymentOption == 'paystack' || $paymentOption == 'iyzico' || $paymentOption == 'paypal' || $paymentOption == 'stripe' || $paymentOption == 'authorize-net' || $paymentOption == 'bitpay' || $paymentOption == 'mercadopago') {
